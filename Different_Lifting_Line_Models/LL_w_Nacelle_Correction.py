...
#Now do an interpolation for swirl angle (psi_i) and incoming flow velocity
...
#Left side trailing edge of all image vortices
w_l=nacelle_radius**2/(lst+dl/2)

#Right side trailing edge of all image vortices
w_r=nacelle_radius**2/(lst-dl/2)
...
for i in range(n_span):
    ...
    for j in range(n_span):
        ...
        A[i][j]=A[i][j]+1/(4*np.pi*velocity_tot)*(-1/(lst[i]-w_l[j])+1/(lst[i]-w_r[j]))                              
        #The line immediately above takes into account of the image vortex
        #And the left side of the vortex (closer to r=0) induces downward flow
        #And the right side of the vortex (closer to the r=Rh) induces upward flow at point i.
...
for i in range(n_span):
    ...
    b[i][0]=twist_dist(lst[i])+psi_i-alpha_0
...
for i in range(len(lst)):
    ...
    for j in range(len(lst)):
        #WRONG (as it incldues the contribution of mirror vortices to thrust): thrust_at_dl.append(circulations[i]*circulations[j]/(4*np.pi)*(-1/(lst[j]-lst[i]+dl/2)+1/(lst[j]-lst[i]-dl/2)-1/(lst[i]-w_l[j])+1/(lst[i]-w_r[j])))
